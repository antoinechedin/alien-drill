...
public class MinecartSpawner : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (timer > 1f / spawnSpeed)
        {
            ...
            Minecart minecart = Instantiate(minecartPrefab, minecartPositions[0], Quaternion.identity, transform);
            minecart.target = minecartPositions[1];
            minecart.transform.LookAt(minecartPositions[1]);
        }
    }
}
