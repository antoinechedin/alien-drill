    ...
    private void Update()
    {
        if (rocks.Count < maxNumOfRocks)
        {
            ...
            if (timer > spawnDuration)
            {
                ...
                Rock rock = Instantiate(rockPrefab[Random.Range(0, maxRockId + 1)], board.RandomPosition(), Quaternion.Euler(0, Random.Range(0f, 360), 0f), transform).GetComponent<Rock>();
